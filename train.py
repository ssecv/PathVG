...
def get_args_parser():
    parser = argparse.ArgumentParser('TransCP: context disentangling and prototype inheriting for robust visual grounding', add_help=False)
    ...
    parser.add_argument('--bert_model', default='pretrained_checkpoints/bert-base-uncased.tar.gz', type=str,
                        help='Bert model')
    parser.add_argument('--bert_token_mode', default='pretrained_checkpoints/bert_base_uncased', type=str, help='Bert tokenizer mode')
    ...
    # parser.add_argument('--resume', default='/home/sse3090/jiangjiwei/TransCP/outputs/pathology_reason_xiaorong/public/checkpoint_best_acc.pth', help='resume from checkpoint')
    ...
    # configure file
    parser.add_argument('--config', default='configs/TransCP_R50_pathology2.py', type=str, help='Path to the configure file.')
...
def main(args):
    ...
    rest_param = [p for n, p in model_without_ddp.named_parameters() if p.requires_grad and
                  (n.startswith('bbox_regression') or n.startswith('bbox_embed') or n.startswith('bert_proj'))]
    ...
    if args.eval:
        print(epoch_eval)
        test_stats, test_time = epoch_eval(model, criterion, postprocessor, data_loader_val, device, args.save_pred_path)

...
class TransCP(nn.Module):
    def __init__(self, pretrained_weights, args=None):
        ...
        # 
        # self.reason_bert = BertModel.from_pretrained(args.bert_model)
        # self.reason_bert_proj = nn.Linear(args.bert_output_dim, args.hidden_dim)
        # self.reason_bert_output_layers = args.bert_output_layers
        # self.reasonmodel = build_LSTMBert(args, bert_model=self.reason_bert)
    ...
    def forward(self, image, image_mask, word_id, word_mask,reason_id_0, reason_mask_0, reason_id_1, reason_mask_1, reason_id_2, reason_mask_2):
        # 1. Image features
        # Eq. (1)
        ...
        # Eq. (2) and Eq. (6) ~ Eq. (8)
        word_id = torch.cat([word_id, reason_id], dim=1)
        word_mask = torch.cat([word_mask, reason_mask], dim=1)
        ...
        # 3. Reason features (including disentangled language features)
        # 把三个reason进行cat操作

        disentangled_lang_reason, bert_fea_reason = self.textmodel(NestedTensor(reason_id, reason_mask))
        word_feat_reason, word_mask_reason = bert_fea_reason.decompose()
        word_feat_reason = self.bert_proj(word_feat_reason)
        word_feat_reason = word_feat_reason.permute(1, 0, 2)
        assert word_mask_reason is not None
        word_mask_reason = word_mask_reason.flatten(1)
        # 3. visual context disentangling + prototype discovery + bbox regression (without box prediction head)
        # Eq. (3) ~ Eq. (14) 
        text_fea = torch.cat([word_feat, word_feat_reason], dim=0)
        text_mask = torch.cat([word_mask, word_mask_reason], dim=1)
        # 消融实验
        hs = self.bbox_regression(img_feat, mask, pos_embed, word_feat, word_mask, projected_disentangled_lang, h, w)
        # hs = self.bbox_regression(img_feat, mask, pos_embed, text_fea, text_mask, projected_disentangled_lang, h, w)
